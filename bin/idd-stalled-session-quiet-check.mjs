#!/usr/bin/env node
// idd-generated-from: src/bin/idd-stalled-session-quiet-check.mts
//
// The bin/idd-stalled-session-quiet-check.mjs copy is generated from the .mts source named above
// by `pnpm run build`. Edit the .mts source, never the generated .mjs.
// See docs/typescript-sources.md.
import { runHelper } from './run-helper.mjs';

runHelper('../scripts/stalled-session-quiet-check.mjs');
