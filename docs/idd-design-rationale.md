...
documented Resume Step 1 invocation (`idd-resume.instructions.md`) to
thread `--claim-id` whenever this session already holds a verified one
for the issue, plus `--nonce` when recorded and `--worktree` once the
B1 worktree exists, so the helper reads independent owner evidence
instead of treating a bare re-fetch as a live competitor. Cold recovery
