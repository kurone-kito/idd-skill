...
- Stable `--acquire` `mode` values: `acquired` (fresh create, a read-only
  ...
  **and** whose top-level `reason` is not a `released-claim-*` reason; a
  ...
  have no claim id and require operator recovery. A `holder` snapshot of
  the previous occupant is reported on **both** a plain `collision` and
  an authorized takeover, not only on takeover.
