...
| Worktree guard          | Advisory only by default — `idd-doctor` warns on a primary-worktree implementation-branch HEAD but nothing blocks the commit, push, or merge                                             | Opt in by setting `worktreeGuard.enabled: true` in `.github/idd/config.json`. When enabled, `idd-doctor --strict` (and a CI step that runs it) fail on a primary-worktree `issue/*` / `roadmap-audit/*` HEAD, and the optional `core.hooksPath` hook rejects such commits/pushes locally. Override `worktreeGuard.branchPatterns` to change which branch globs count as implementation branches (default `issue/*`, `roadmap-audit/*`). Absent or `false` keeps the historical advisory-only behavior.                                    |
