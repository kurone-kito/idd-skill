...
| Upstream escalation                 | Absent or `false` — this document records the `upstreamEscalation.enabled` policy-field surface only; it has no effect by itself yet                                                                                                                                                                                                                                                                                                                                                      | `schemas/policy.schema.json` already validates `upstreamEscalation.enabled`, but no phase file reads it yet — the detection/routing/authoring behavior it will gate lands in the [`upstream-candidate-flag` roadmap](https://github.com/kurone-kito/idd-skill/issues/2700)'s other tracks. Do not set it expecting behavior yet; setting it today is inert (unlike `instructionProfile` below, it does not fail schema validation). Never set this in `kurone-kito/idd-skill`'s own `.github/idd/config.json` once it does take effect: this repository is the upstream source itself, so the adopter-only condition it will gate cannot occur here.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
