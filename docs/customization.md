...
| Upstream escalation                 | Absent or `false` (default) reproduces today's behavior exactly; `true` gates `audit-authored-issue.mjs`'s `upstream-candidate-marker-label` check (#2703)                                                                                                                                                                                                                                                                                                                                | `schemas/policy.schema.json` validates `upstreamEscalation.enabled`; `audit-authored-issue.mjs` (#2703) now reads it to gate the `status:upstream-candidate` label / `{marker-prefix}-upstream-candidate` marker pairing check -- absent or `false` keeps that check a no-op. The detection/routing that actually produces the marker/label lands in the [`upstream-candidate-flag` roadmap](https://github.com/kurone-kito/idd-skill/issues/2700)'s remaining track (#2702). Never set this in `kurone-kito/idd-skill`'s own `.github/idd/config.json`: this repository is the upstream source itself, so the adopter-only condition it gates cannot occur here.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
