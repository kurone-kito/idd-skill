...
dogfooding work for that bundle, a generated byte-identical mirror under
...
2. **Per-phase skills** (discover, claim, work, review, merge, resume),
   each wrapping one phase file.
...
surfaces (Copilot, Codex CLI, Claude Code, Gemini CLI) consume
identically today. The artifact taxonomy in
...
- **Generated mirror**: mechanically possible today — the
  `.claude/skills/issue-authoring` mirror shows the pattern (four
