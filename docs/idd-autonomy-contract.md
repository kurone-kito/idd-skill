...
- **Reversible**: the loop runs it without a dedicated pre-approval
  gate beyond the ordinary
  [claim revalidation gate](../.github/instructions/idd-overview-core.instructions.md#claim-revalidation-gate).
  The row names the undo command or reversal path.
- **Irreversible**: the loop only runs it after a dedicated, named
  gate — a merge-readiness checklist, a human-gated policy, or (for a
  small set of GitHub-minimize actions) because no IDD instruction
  path ever reverses it. The row names the governing gate or the
  reason no reversal path exists.
...
| Stale-claim takeover (`claimed-by`, `supersedes: <prior>`)      | Reversible                | Post `unclaimed-by`; a later session can claim fresh once released                                                                                                                         | A5 pre-check (c); gated by the 24 h `claim-stale-age` threshold and race-safe verification before the takeover post is allowed |
...
| Create or link a follow-up issue for an autonomous gap             | Reversible                | Close the created issue                                                                                                                                                  | `idd-roadmap-audit.instructions.md` "Autonomous gaps found"; gated by a narrow duplicate/reuse check before creation |
...
| Merge `main` into the feature branch (post-publication sync) | Reversible before its own push | `git merge --abort` / local reset before pushing; once pushed it joins Push below | E11, E-phase branch-sync check (`idd-review-fix.instructions.md`, `idd-review-triage.instructions.md`) |
...
| Resolve a review thread                                                               | Reversible (reviewer-initiated) | The reviewer can reopen it; the agent must not undo its own resolution unilaterally      | E6, E13 (`idd-review-triage.instructions.md`, `idd-review-fix.instructions.md`) |
