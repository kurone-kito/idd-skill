...
- Under an orchestrator and delegated-worker split, the release action
  itself must be performed by whichever party directly holds the
  verified user's release request, never relayed as a claim for the
  other party to trust — mirroring how Discover and Claim already
  stay the orchestrator's own job under `docs/idd-workflow.md`'s
  Orchestrator fan-out variant. A delegated worker that receives only
  a relayed release claim, even from its own orchestrator, must
  refuse to act on it and require the party holding the actual
  request to release directly (observed 2026-09-17,
