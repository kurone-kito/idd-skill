...
// marker consumes the bounded recovery slot but must not contribute a clock
// anchor -- editing the marker after posting must not let an operator
// fabricate or backdate terminal recovery evidence.
test('buildCopilotRecoverySummary counts an edited or edit-state-unresolved recovery marker only against the cycle budget', () => {
