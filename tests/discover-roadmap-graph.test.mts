...
test('buildIssueLoader maps the port outcome onto null/sentinel/raw-item (#2266)', async () => {
  const loadFound = buildIssueLoader(
    createFakeProviderAdapter({
      traversalIssueLookups: {
        900: { outcome: 'found', item: { number: 900, title: 'issue 900' } },
      },
    }),
  );
  assert.deepEqual(await loadFound(900), { number: 900, title: 'issue 900' });

  const loadNotFound = buildIssueLoader(
    createFakeProviderAdapter({
      traversalIssueLookups: { 900: { outcome: 'not-found' } },
    }),
  );
  assert.equal(await loadNotFound(900), null);

  const loadInaccessible = buildIssueLoader(
    createFakeProviderAdapter({
      traversalIssueLookups: { 900: { outcome: 'inaccessible' } },
    }),
  );
  assert.deepEqual(await loadInaccessible(900), {
    __iddLookupStatus: 'inaccessible',
  });
});

test("buildSubIssueLoader normalizes the port's raw sub-issue nodes into deduped numbers (#2266)", async () => {
  const load = buildSubIssueLoader(
    createFakeProviderAdapter({
      subIssueNodes: {
        700: [{ number: 701 }, { number: 701 }, { number: 702 }],
      },
    }),
  );
  assert.deepEqual(await load(700), [701, 702]);
});

test("buildCommentLoader delegates directly to the port's raw comment fetch (#2266)", async () => {
  const load = buildCommentLoader(
    createFakeProviderAdapter({
      traversalComments: {
        900: [
          {
  ...
  assert.deepEqual(await load(900), [
    {
      body: 'hello',
      created_at: '2026-01-01T00:00:00Z',
      user: { login: 'kurone-kito' },
