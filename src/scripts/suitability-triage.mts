...
interface NormalizedIssue {
  ...
  createdAt: string;
  /** #2767: the issue author's login, read only for the live CLI's
   * `trustedEditor` structural-evidence signal. `''` in
   * `evaluateSuitabilityLocal`'s `--body-file`/`--stdin` mode, which has
   * no live author to report -- `isTrustedEditorSignal` already treats an
   * empty author as untrusted, so this alone keeps local mode from ever
   * demoting. */
  author: string;
}
...
interface SuitabilityOptions {
  ...
  /** #2767 -- see `Context.structuralEvidence`'s doc comment. Accepted
   * as `unknown` like every other option here; normalized (or dropped
   * if malformed) before being copied into `Context`. */
...
function findRepositoryFitHiddenMetadataRanges(
  text: string,
): MarkdownCodeRange[] {
  const ranges: MarkdownCodeRange[] = [];
  const hasLinkLabel = (closeIndex: number) => {
    let nestedClosers = 0;
  ...
  const pushBalancedLinkDestination = (openIndex: number) => {
    let depth = 1;
...
function buildRepositoryFitParagraphSpans(
  scanBody: string,
  nonInlineCodeRanges: readonly MarkdownCodeRange[],
  ...
  const tableRows = new Set<(typeof tableLines)[number]>();
...
export function checkRepositoryFit(context: Context): CheckOutcome {
  ...
  const lowerCaseScanBody = externalAccessScanBody
    .replace(
      /(?<![.\w])([A-Za-z]{2,})\.(?=[ \t]+[A-Z])/g,
      (_whole: string, word: string, offset: number, source: string) => {
  ...
  const listItemContextFor = (matchIndex: number) => {
  ...
  const hasIndependentConjunctionClause = (text: string) =>
...
export function checkVerifiability(context: Context): CheckOutcome {
  ...
  const subjectiveApprovalMatch = ((): string | null => {
    let lineOffset = 0;
...
function runCli(): HelperCliResult {
  ...
  const suitabilityOptions: SuitabilityOptions = {
...
function normalizeStructuralEvidence(
  raw: unknown,
  ...
  const r = raw as {
    ...
    candidateFilesExist?: unknown;
    trustedEditor?: unknown;
  };
  if (
 ...
 */
function fetchUserContentEditors(
  owner: string,
  repo: string,
  issueNumber: number,
): (string | null)[] {
  const allNodes: { editor?: { login?: unknown } | null }[] = [];
  ...
  repository(owner:$owner,name:$repo){
    ...
    const apiArgs = [
      ...
      'graphql',
      ...(resolveGhApiHostname()
        ? ['--hostname', resolveGhApiHostname() as string]
        : []),
      '-f',
      `query=${query}`,
      '-f',
    ...
    const parsed = ghJson(apiArgs) as {
      data?: {
        repository?: {
          issue?: {
            userContentEdits?: {
              ...
              nodes?: { editor?: { login?: unknown } | null }[] | null;
            } | null;
          } | null;
        } | null;
      };
