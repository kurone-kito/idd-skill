...
} from './suitability-triage.mts';

export type { AuthoringBucketMarkerDetection, AuthoringBucketMarkerValue };
...
// AuthoringBucketMarkerValue and AuthoringBucketMarkerDetection moved to
// marker-helpers.mts (#3289, see that module's "Shared markerPrefix /
// marker-occurrence helpers" section) and are re-exported above.
...
export interface AuditFinding {
   ...
   * below.
   *
   * A fourth, differently-shaped source (#3289): under `--expect-bucket`,
   * any `triage-a4-*`/`triage-a45-*` finding that would otherwise have
   * failed is rewritten to `result: 'pass'` with this field set, via
   * `demoteIfBucketAudit` in `buildTriageFindings`. Unlike the three
   * always-pass advisory checks above, this finding's underlying
   * evaluation genuinely failed -- the demotion is conditional on
   * `--expect-bucket` (a needs-decision/blocked-by-human body is meant to
   * be non-ready), not an inherent property of the check itself.
   */
  severity?: 'warning';
}
...
export interface AuditOptions {
  ...
  markerPrefix?: string;
  /**
   * The drafted issue's title, for the `triage-a4-*` / `triage-a45-*`
   * checks (#3289) -- those run the same `evaluateA4Viability` /
   * `evaluateSuitabilityLocal` evaluators Discover and
   * `suitability-triage.mjs --body-file` use, and both need a title
   * separate from the body. Only consulted for the `orphan` and `child`
   * shapes (Discover never routes a `roadmap` node through A4 or A4.5, so
   * `buildTriageFindings` reports every triage finding as not applicable
   * there regardless of `title`). When omitted or empty/whitespace-only, a
   * leading `# <title>` line of `body` is used instead, following the
   * same `splitLocalDraftTitleAndBody` convention
   * `suitability-triage.mts`'s own local dry-run already uses; when
   * neither is present, `triage-title-missing` fails and every
   * `triage-a45-*` finding reports "not evaluated" (an empty title always
   * fails Check 2/Coherence, so running the other five would only add
   * noise) while `triage-a4-*` still evaluates normally (its three
   * criteria are title-independent).
   */
  title?: string;
  /**
...
function buildTriageFindings(
  ...
  const demoteIfBucketAudit = (finding: AuditFinding): AuditFinding =>
    isBucketAudit && finding.result === 'fail'
      ? { ...finding, result: 'pass', severity: 'warning' }
      : finding;

  const titleFinding = ((): AuditFinding => {
  ...
  const a4Findings = a4Result.criteria.map((criterion): AuditFinding => {
  ...
  const a45Findings = suitability.checks.map((check): AuditFinding => {
