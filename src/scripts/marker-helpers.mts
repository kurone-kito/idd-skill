...
export interface OperationalMarker {
   ...
   * `zero-accepted-path-a-gate`); `advisory-wait`, `forced-handoff`, and
   * `idd-external-check-waiver` do not use the shared note-optional grammar
   * this field targets.
   */
...
export interface LegacyClaimEventLike {
  body?: string | null;
  createdAt?: string | null;
}

/** Resolved legacy claim/release pair from normalized comment events. */
export interface LegacyClaimState {
  claim: ParsedLegacyClaimMarker | null;
  released: boolean;
  releasedClaim: ParsedLegacyClaimMarker | null;
}
...
/**
 * Parsed `<!-- idd-provider-outage-park: ... -->` marker (#2321): posted on
 * the parked pull request itself (unlike the declaration-target-scoped
 * markers above) when its sole blocker is a `providerHealth` verdict of
 * `unavailable` for `service`. `issueNumber` names the originating issue
 * whose claim was released on entry, so a later resume re-claims the right
 * target without re-parsing the pull request body's closing keyword.
 */
export interface ParsedProviderOutagePark {
  actor: string;
  issueNumber: number;
  service: string;
  headSha: string;
...
export function parseLegacyClaimComment(
  body: string,
  createdAt: string,
): ParsedLegacyClaimMarker | null {
...
export function parseProviderOutageParkComment(
  body: string,
  createdAt: string,
