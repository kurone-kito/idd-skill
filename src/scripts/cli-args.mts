...
import { markCliUsageError } from './helper-cli-runner.mts';
...
/**
 * One flag's declarative spec. `type`/`short`/`multiple`/`default` are
 * passed through verbatim to `util.parseArgs` (after this module strips
 * the map key's leading `--`). See the module header for why the map key
 * itself must stay the dashed literal.
 */
...
/** Declarative flag spec, keyed by the dashed CLI literal (e.g. --pr). */
export type CliFlagSpecMap = Record<string, CliFlagSpec>;

/** A single parsed flag value, mirroring `util.parseArgs`'s own value shape. */
export type CliFlagValue = string | boolean | string[] | boolean[] | undefined;

/** Result of {@link parseCliArgs}. */
export interface CliParseResult {
  /** Parsed values, keyed by the flag's bare name (dashes preserved,
   * leading `--` stripped) -- e.g. spec key --claim-issue reads back as
   * `values['claim-issue']`. */
  values: Record<string, CliFlagValue>;
  /** Positional arguments. Always `[]` today: this wrapper parses with
   * `allowPositionals: false`, matching every existing hand-rolled parser
   * (none of which accept bare positional tokens); a stray positional
   * throws `unknown argument: <token>` instead of being collected here. */
  positionals: string[];
 ...
 * against.
 */
function toRepoShapedError(error: unknown): Error {
  if (
...
export function parseCliArgs(
  ...
  return {
    values: parsed.values as Record<string, CliFlagValue>,
    positionals: parsed.positionals,
    help: Boolean((parsed.values as Record<string, CliFlagValue>).help),
  };
}
...
function parseCanonicalIntegerToken(
 ...
 */
export function parseCanonicalIntegerOrThrow(
  token: string | undefined,
  flagName: string,
  min = 1,
): number {
 ...
 */
export function parseCanonicalIntegerOrNull(
  token: string | undefined,
  min = 1,
): number | null {
