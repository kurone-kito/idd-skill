...
function countInterruptingCheckboxItems(sectionText: string): number {
  ...
  for (const line of lines) {
    ...
    if (BLOCKQUOTE_START_LINE_PATTERN.test(line)) {
      ...
      if (quotedContent.trim() === '') {
        ...
        if (quotedListItemMatch) {
          ...
          const quotedOpensOrContinues: boolean =
    ...
    let currentLineOpensOrContinuesList: boolean;
    if (listItemMatch) {
...
function startsFreshParagraph(body: string, index: number): boolean {
...
/** One recognized Acceptance Criteria heading's position within the body
 * {@link findAcceptanceCriteriaHeadings} scanned: `index` is the offset of
 * its first character, `length` is the full match length (including a
 * Setext heading's own underline line), so a caller can compute
 * `index + length` as the section's content start. */
export interface AcceptanceCriteriaHeadingMatch {
  index: number;
  length: number;
}
 ...
 * a previous scan.
 */
export function findAcceptanceCriteriaHeadings(
  body: string,
  options: { includeBoldPseudoHeading?: boolean } = {},
): AcceptanceCriteriaHeadingMatch[] {
  const patterns = [
  ...
  const matches: AcceptanceCriteriaHeadingMatch[] = [];
...
function extractSection(
  body: string,
  headingMatch: AcceptanceCriteriaHeadingMatch | null,
