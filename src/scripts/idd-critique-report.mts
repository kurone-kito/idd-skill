...
import type { CritiqueTelemetrySample } from './idd-critique-harvest.mts';
...
export interface CritiqueTelemetrySnapshot {
  schemaVersion: 1;
  generatedOn: string;
  sampleCount: number;
  ...
  delegateUsageCount: number;
  delegateUsageRate: number | null;
  minPublishableSamples: number;
  publishable: boolean;
}
 ...
 */
export function assertCritiqueTelemetrySnapshot(
  snapshot: unknown,
): asserts snapshot is CritiqueTelemetrySnapshot {
...
export function readCritiqueSamples(
  ...
  for (const path of paths) {
    for (const { lineNumber, text } of readJsonlLines(path)) {
      ...
      samples.push(sample as unknown as CritiqueTelemetrySample);
    }
  }
  return samples;
}
 ...
 */
export function aggregateCritiqueSnapshot(
  samples: readonly CritiqueTelemetrySample[],
  now: Date,
): CritiqueTelemetrySnapshot {
  let totalFindings = 0;
  let high = 0;
  let medium = 0;
  let low = 0;
...
export function renderDocsTableRegion(
  snapshot: CritiqueTelemetrySnapshot,
): string {
 ...
 */
export function checkRenderedFiles(
  snapshot: CritiqueTelemetrySnapshot,
  docsPath: string,
): string[] {
  ...
  try {
    ...
    return [`${docsPath}: ${(error as Error).message}`];
  }
  if (expected !== content) {
    drifted.push(
...
//
// Mirrors token-cost-report.mts's own #2452 guard: refuse --apply while
// the current branch is the repository's default branch, since a stray
// --apply left dirty on the shared primary worktree can block every
// concurrent session's next B1 `git merge --ff-only` worktree creation.
// Reimplemented locally rather than imported -- see this file's own
// header comment for why.
// ---------------------------------------------------------------------------

interface DefaultBranchGuardDeps {
...
if (import.meta.main) {
  ...
  const docsPath = values.docs as string;
  let now = new Date();
  if (values.now) {
