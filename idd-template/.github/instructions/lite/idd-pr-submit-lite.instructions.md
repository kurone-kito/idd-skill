...
3. The PR body must include: a concise summary, a closing keyword line
   ...
   speculate. The prose sections follow the resolved
   `authoringLanguage` value from `.github/idd/config.json` (fixed
   tag, the claimed issue's own body language for `match-source`, or
