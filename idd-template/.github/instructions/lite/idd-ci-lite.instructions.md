...
tool's own per-invocation timeout, not a utility inside the command
(e.g. Claude Code's Bash `timeout`; issue `#3449`) — near the
tool's ceiling, not its default (`#2933`). Never blindly re-issue
