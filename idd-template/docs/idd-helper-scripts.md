...
- When helper runtime is enabled, Resume/S2 should call the
  ...
  --now <server-anchored-ISO8601>` command first (see
  `idd-resume-stall.instructions.md` for how to derive the
  server-anchored value). `node scripts/stalled-session-quiet-check.mjs
  --pr <pr-number> --now <server-anchored-ISO8601>` is the vendored
  equivalent.
