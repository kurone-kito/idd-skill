...
import { resolveGhHostnameArgs } from './minimize-superseded-markers.mjs';
 ...
 */
export function inspectLastEditedAt(comment) {
  const raw = comment.lastEditedAt;
  if (raw === null) {
    return null;
  }
...
function graphqlIncompleteEvidence(detail) {
...
export function fetchProvenanceCommentsGraphql(owner, repo, issueNumber) {
  ...
  while (true) {
    ...
    try {
      ...
      throw new Error(
        graphqlIncompleteEvidence(`gh-graphql-parse: ${error.message}`),
      );
    }
...
function notFoundGraphqlResult(target, liveBody, detail) {
  const computedBodySha256 = createHash('sha256')
