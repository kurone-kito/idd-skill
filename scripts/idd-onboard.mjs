 ...
 */
function readTargetPolicyConfig(targetDir) {
  if (
    hasNonDirectoryAncestor(targetDir, '.github/idd/config.json') ||
    !fileExists(targetDir, '.github/idd/config.json')
  ) {
 ...
 */
function escapeActionsExpressionStringLiteral(value, context) {
 ...
 */
export function buildUntrustedLabelerGuardWorkflowContent(labels) {
  const labelFields = [
 ...
 */
function assertSafeGuardWorkflowDestination(targetDir, path) {
 ...
 * (#2684 review).
 */
export function planUntrustedLabelerGuardWorkflow(targetDir) {
 ...
 * `--verify` does not run.
 */
export function checkPackagePinWarning(targetRoot) {
...
function isSchemaOrFixtureManifestPath(targetPath) {
  return (
 ...
 */
function listTargetScriptModules(targetRoot) {
...
export async function runHearWizard(catalog, targetDir, options = {}) {
  ...
  async function runHearWizardBody() {
