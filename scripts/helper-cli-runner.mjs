 ...
 */
export function markCliUsageError(error) {
  if (
    error &&
    typeof error === 'object' &&
...
function isMarkedCliUsageError(error) {
  return Boolean(
    error && typeof error === 'object' && error[CLI_USAGE_TAG] === true,
  );
}
function isGhCommandError(error) {
  return Boolean(
    error && typeof error === 'object' && error.ghCommand === true,
  );
}
function errorMessage(error) {
  return error instanceof Error ? error.message : String(error);
}
function ghCommandErrorMessage(error) {
 ...
 */
export function classifyHelperError(error) {
...
export function buildHelperErrorEnvelope(helperName, exitCode, classified) {
  return {
    iddHelperError: {
      version: 1,
      helper: helperName,
...
function normalizeOutcome(helperName, outcome) {
 ...
 */
export function isHelperErrorEnvelopeEnabled(env = process.env) {
 ...
 */
export function writeStderrSync(text) {
...
export const DEFAULT_HELPER_CLI_IO = {
  env: process.env,
...
function envelopeLineFor(io, helperName, exitCode, classified) {
  if (exitCode === 0 || !isEnvelopeEnabled(io.env)) {
    return null;
  }
  return `${JSON.stringify(buildHelperErrorEnvelope(helperName, exitCode, classified))}\n`;
}
/**
 ...
 * throwing.
 */
function handleThrown(helperName, io, error) {
 ...
 * genuinely last.
 */
function handleOutcome(helperName, io, outcome) {
 ...
 */
export function runHelperCli(helperName, main, io = DEFAULT_HELPER_CLI_IO) {
  let result;
  try {
    result = main();
  } catch (error) {
