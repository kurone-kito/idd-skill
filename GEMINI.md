...
- **Helper sources**: the helper logic is migrating to TypeScript wave
  by wave. For a helper that has a `src/**/*.mts` source, that `.mts` is
  the source of truth and its `scripts/*.mjs` / `bin/*.mjs` artifact is
  generated by `pnpm run build` and committed — edit the `.mts`, never
  the generated `.mjs`. Helpers not yet converted remain hand-edited
  `.mjs`. See [docs/typescript-sources.md](docs/typescript-sources.md).
