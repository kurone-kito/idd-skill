...
7. Publish each `ready` drafted body directly under the authoring hold
   ...
   - The originating Stage 1 hold uses this append-only publication-intent
     ...
     comment-only issue). Do not create that journal as part of the same
     set. If `issueAuthoring.journalIssue` is not configured, or its
     identity cannot be verified, stop with `blocked-by-human` before
     creating any target. On every paginated replay, require `actor` to
     equal the API author and
